/**
 * @file hal_uart.h
 * @brief UART HAL (C6) — public interface for the C6↔H2 bridge link.
 *
 * Logical byte-stream access to the bridge UART. The HAL owns the port, pins
 * (config UART_BRIDGE_TX/RX_GPIO), baud, and ring buffers; callers (the
 * zigbee_bridge) only read/write bytes and never include `driver/uart.h`.
 * C6 side of the HAL boundary (hal-design.md).
 */
#ifndef HAL_UART_H
#define HAL_UART_H
