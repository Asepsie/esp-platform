/**
 * @file hal_uart.c
 * @brief UART HAL — ESP32-H2 target implementation (bridge link).
 *
 * The ONLY file on the H2 that includes `driver/uart.h`. Owns the UART1 setup
 * (port, pins, baud, ring buffers) and exposes byte read/write to the bridge.
 */
#include "hal_uart.h"
...
// UART1 — UART0 is the log console (see sdkconfig.defaults).
...
esp_err_t hal_uart_init(void)
{
    ...
    if (err != ESP_OK) return err;
    return uart_set_pin(HAL_UART_PORT, HAL_UART_TX_GPIO, HAL_UART_RX_GPIO,
